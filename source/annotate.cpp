...
std::string reverse_compliment ( const std::string &sequence ) {
	auto compliment_character = [&]( const char &nucleotide ){
	...
	string reverse;
	std::transform( sequence.begin(), sequence.end(), reverse.begin(), compliment_character );
	return reverse;
}
...
void inline getRevKmers (kmer_map& kmers) {
	for (unordered_map<string,string>::iterator it = kmers.all.begin(); it != kmers.all.end(); it++) {
		kmers.rev.emplace(reverse_compliment(it->first), it->second);
	}
