...
//STP: Added for compatibility with new clouds
		...
		while (!feof(pfRepeatfile))
		{
			if( fgets( pchLine, MAXLINECHAR, pfRepeatfile ) != NULL)
			{
				...
				*(pchTemp + size) = '\0';
				...
				if ( 1 or ((isonessr(pchTemp) == 0) && (istwossr(pchTemp) == 0) && (isthreessr(pchTemp) == 0) && (isfourssr(pchTemp) == 0) ))
				{
...
//		if (Repeat1[i].cloud > 4) {
//			cout << "core kmer " << i << " has cloud " << Repeat1[i].cloud << endl;
//		}
...
int pcloudsdissection(const char* pchControlfile)
{
	...
	else
	{
		if (m_nGetclouds)
		{
			if (readrepeatnumber(pchrepeatfile, number1, number2, size, m_nStep1, m_nEndthreshold, m_nCopy))
			{
				cloud_type3* repeats1 = (cloud_type3*) calloc(number1, sizeof(cloud_type3));
                                readmainoligos(pchrepeatfile, repeats1, number1, size, m_nStep1, m_nEndthreshold);
